...
type DayToggleCircleProps = {
  ...
  showCurrentDaySlot?: boolean;
};

function DayToggleCircle({
  letter,
  checked,
  onToggle,
  readOnly = false,
  isCurrentDay = false,
  showCurrentDaySlot = false,
}: DayToggleCircleProps) {
    ...
    >
      ...
      {showCurrentDaySlot ? (
        <span className="relative h-8 w-4 shrink-0" aria-hidden>
          {isCurrentDay ? (
            <Image
              src={CHILD_DASHBOARD_ASSETS.currentDayDot}
              alt=""
              width={16}
              height={32}
              className="absolute inset-0 h-8 w-4"
              unoptimized
            />
          ) : null}
        </span>
      ...
      <div className="flex w-full items-center justify-between">
        {DAY_LETTERS.map((letter, dayIndex) => (
          <DayToggleCircle
            ...
            showCurrentDaySlot={showCurrentDaySlot}
          />
        ))}
      </div>
    </section>
...
type DashboardWeekTrackerProps = {
  ...
  /** Child dashboard — view only; parent marks days. */
  readOnly?: boolean;
  /** Child copy: "השינוי הראשון שלי". Parent: "השינוי הראשון של {name}". */
  titleVariant?: 'parent' | 'child';
  /** White dot under today's Hebrew weekday (א…ש). */
  showCurrentDayDot?: boolean;
  onActiveChecksChange?: (checked: boolean[]) => void;
};
...
function buildCardsFromChanges(
  ...
  titleVariant: 'parent' | 'child' = 'parent'
): ChangeCardMock[] {
...
export function DashboardWeekTracker({
  ...
  readOnly = false,
  titleVariant = 'parent',
  showCurrentDayDot = false,
  onActiveChecksChange,
}: DashboardWeekTrackerProps) {
  const cards = cardsProp ?? buildCardsFromChanges(
    childName,
    changes,
    changeDayChecks,
    changeText,
  ...
  const persistChecks = useCallback(
    async (nextByCard: Record<string, boolean[]>) => {
      if (readOnly || !childId || !parentId || cards.length === 0) return;
    ...
    [childId, parentId, cards, readOnly]
  );
  ...
  useEffect(() => {
    if (!onActiveChecksChange) return;
    const card = cards[activeIndex];
    onActiveChecksChange(
      card ? checkedByCard[card.id] ?? emptyChecks() : emptyChecks()
    );
  }, [activeIndex, checkedByCard, cards, onActiveChecksChange]);

  if (cards.length === 0) return null;

  const currentDayIndex = showCurrentDayDot ? israelSundayBasedDayIndex() : null;
        ...
        {cards.map((card, index) => (
          ...
          >
            <ChangeCard
              ...
              showCurrentDaySlot={showCurrentDayDot}
            />
          </div>
        ))}
      </div>
      ...
      >
        {cards.map((card, index) => {
          ...
          return (
            ...
            >
              <span
                className="block rounded-full transition-[background-color,transform] duration-300 ease-out"
                style={{
                  width: dotSize,
                  height: dotSize,
