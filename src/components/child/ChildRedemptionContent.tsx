...
export function ChildRedemptionContent({
  ...
  const handleConfirmAndProcess = async () => {
    ...
    try {
      ...
      const imageRef = ref(storage, `weekly-uploads/${challengeId}/${Date.now()}.jpg`);
      await uploadBytes(imageRef, uploadedImage);
      const screenshotUrl = await getDownloadURL(imageRef);

      let processedData: { screenTimeMinutes: number; apps?: Array<{ name: string; timeUsed: number; icon?: string }> } | undefined;
      try {
      ...
      <div className="min-h-screen bg-transparent pb-24 flex items-center justify-center">
        <div className="max-w-md mx-auto px-4 py-8">
          <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6 text-center">
            <h1 className="font-varela font-semibold text-2xl text-[#262135] mb-4">כתובת לא תקינה</h1>
            <p className="font-varela text-base text-[#282743] mb-4">{urlError || 'הכתובת ששותפה איתך לא תקינה או שהפדיון הושלם כבר.'}</p>
      ...
      <div className="min-h-screen bg-transparent pb-24 flex items-center justify-center">
        <div className="max-w-md mx-auto px-4 py-8">
          <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6 text-center">
            ...
            <h1 className="font-varela font-semibold text-2xl text-[#262135] mb-4">הפדיון בוצע בהצלחה!</h1>
            <p className="font-varela text-base text-[#282743] mb-4">לשבוע הבא, ההורה ישלח לך כתובת חדשה.</p>
  ...
  return (
    <div className="min-h-screen bg-transparent pb-24 relative">
      {isProcessing && (
        <div className="fixed inset-0 bg-white bg-opacity-80 flex items-center justify-center z-50">
          <div className="text-center">
            <div className="font-varela text-lg text-[#262135] mb-4">
      ...
      <div className={`max-w-md mx-auto px-4 py-8 ${isProcessing ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="text-center mb-6 mt-16">
          <h1 className="font-varela font-semibold text-2xl text-[#262135]">יום הפדיון! 🎉</h1>
          <p className="font-varela text-sm text-[#948DA9] mt-1">{redemptionDate && `תאריך: ${redemptionDate}`}</p>
        </div>
        ...
        {currentStep === 'estimate' && (
          <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6">
            ...
            <div className="mb-6">
              <p className="font-varela text-base text-[#282743] mb-3 text-center">האם לדעתך עמדת ביעד השבוע?</p>
              <div className="flex gap-4 justify-center">
                ...
                >
                  ...
                  <p className="font-varela font-semibold mt-1">כן!</p>
                </button>
                <button
                ...
                >
                  <span className="text-3xl">👎</span>
                  <p className="font-varela font-semibold mt-1">לא</p>
                </button>
              </div>
            </div>
            <div className="mb-6">
              <p className="font-varela text-base text-[#282743] mb-3 text-center">כמה כסף {childP.think} שצברת?</p>
              <div className="bg-gradient-to-r from-[#E6F19A] to-[#BBE9FD] rounded-[12px] p-4">
                <input
                  type="range"
                  min="0"
                ...
                <div className="text-center mt-3">
                  <span className="font-varela font-bold text-3xl text-[#262135]">₪{childEstimatedEarnings}</span>
                  <p className="font-varela text-xs text-[#948DA9] mt-1">מתוך ₪{challenge?.selectedBudget || 100} אפשריים</p>
                </div>
              </div>
            </div>
            <button
        ...
        {currentStep === 'upload' && (
          <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6">
            <h2 className="font-varela font-semibold text-xl text-[#262135] mb-4 text-center">העלאת צילום מסך</h2>
            <p className="font-varela text-base text-[#282743] mb-4 text-center">יחד עם {parentName}, העלו צילום מסך של זמן המסך השבועי</p>
            <div className="mb-4">
              <p className="font-varela text-sm text-[#948DA9] mb-2 text-center">מי מעלה?</p>
              <div className="flex gap-2 justify-center">
                <button
                  onClick={() => setUploadedBy('child')}
                  className={`px-4 py-2 rounded-full text-sm font-varela transition-all ${
            ...
            <div className="border-2 border-dashed border-[#BBE9FD] rounded-[18px] p-8 text-center mb-6 relative">
              {imagePreview ? (
                ...
                <>
                  ...
                  <p className="font-varela text-[#282743] mb-2">לחצו להעלאת תמונה</p>
                  <p className="font-varela text-xs text-[#948DA9]">או גררו תמונה לכאן</p>
                </>
              )}
              <input
                type="file"
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-4 px-6 rounded-[18px] text-lg font-varela font-semibold border-2 border-[#273143] text-[#273143] hover:bg-gray-50 transition-all"
              >
                חזרה
              </button>
              <button
        ...
        {currentStep === 'preview' && (
          <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6">
            <h2 className="font-varela font-semibold text-xl text-[#262135] mb-4 text-center">אישור לפני עיבוד</h2>
            <div className="bg-gray-50 rounded-[12px] p-4 mb-4">
              <p className="font-varela text-sm text-[#948DA9] mb-2">ההערכה שלך:</p>
            ...
            {imagePreview && (
              <div className="mb-4">
                <p className="font-varela text-sm text-[#948DA9] mb-2">התמונה שהעלית:</p>
                <img src={imagePreview} alt="Screenshot" className="w-full rounded-[12px] border border-gray-200" />
              </div>
            )}
            <p className="font-varela text-sm text-[#948DA9] text-center mb-4">לאחר האישור, התמונה תעבור עיבוד וההורה יקבל הודעה לאישור.</p>
            <div className="flex gap-3">
              <button
              ...
              <button onClick={handleConfirmAndProcess} className="flex-1 py-4 px-6 rounded-[18px] text-lg font-varela font-semibold bg-[#273143] text-white hover:bg-opacity-90 transition-all">
                אשר ושלח
              </button>
            </div>
          </div>
        ...
        {currentStep === 'results' && (
          <div className="bg-gradient-to-br from-[#E6F19A] to-[#BBE9FD] rounded-[18px] shadow-card p-6 text-center">
            {weeklyUploadStatus === 'pending' ? (
              <>
                <div className="text-6xl mb-4">⏳</div>
                <h2 className="font-varela font-semibold text-xl text-[#262135] mb-4">ממתין לאישור {parentName}</h2>
                ...
                <div className="bg-white bg-opacity-80 rounded-[12px] p-4 mb-4">
                  <p className="font-varela text-sm text-[#948DA9] mb-1">ההערכה שלך:</p>
                  <p className="font-varela font-bold text-xl text-[#262135]">₪{childEstimatedEarnings}</p>
                </div>
                <p className="font-varela text-sm text-[#948DA9]">העמוד יתעדכן אוטומטית לאחר האישור</p>
              </>
            ) : (
              <>
                <div className="text-6xl mb-4">🎉</div>
                <h2 className="font-varela font-semibold text-xl text-[#262135] mb-4">התוצאות שלך!</h2>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="bg-white bg-opacity-80 rounded-[12px] p-4">
                    <p className="font-varela text-sm text-[#948DA9] mb-1">{childP.thought}:</p>
                    <p className="font-varela font-bold text-xl text-[#282743]">₪{childEstimatedEarnings}</p>
                  </div>
                  <div className="bg-white bg-opacity-80 rounded-[12px] p-4">
                    <p className="font-varela text-sm text-[#948DA9] mb-1">בפועל:</p>
                    <p className="font-varela font-bold text-xl text-[#262135]">₪{actualEarnings}</p>
                  </div>
                </div>
        ...
        {currentStep === 'redemption' && (
          <>
            <div className="bg-gradient-to-br from-[#E6F19A] to-[#BBE9FD] rounded-[18px] shadow-card p-6 mb-6 text-center">
              <h2 className="font-varela font-semibold text-xl text-[#262135] mb-2">{parentName} {parentP.approved}! 🎉</h2>
              <p className="font-varela text-base text-[#282743] mb-4">{childData.childName}, הרווחת השבוע:</p>
              <div className="bg-white bg-opacity-80 rounded-[12px] p-4">
                <p className="font-varela font-bold text-3xl text-[#262135]">₪{actualEarnings}</p>
              </div>
            ...
            <div className="bg-[#FFFCF8] rounded-[18px] shadow-card p-6 mb-6">
              <h2 className="font-varela font-semibold text-lg text-[#262135] mb-4 text-center">איך {childP.wants} לקחת את הכסף?</h2>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-varela font-semibold text-base text-[#282743] mb-1">{option.label}</h3>
                        <p className="font-varela text-sm text-[#948DA9] whitespace-pre-line">{option.description}</p>
                      </div>
        ...
        <div className="mt-6 bg-[#FFFCF8] rounded-[18px] shadow-card p-4 text-center">
          <p className="font-varela text-xs text-[#948DA9]">תאריך הפדיון: {redemptionDate || 'טוען...'}</p>
        </div>
      </div>
    </div>
  );
