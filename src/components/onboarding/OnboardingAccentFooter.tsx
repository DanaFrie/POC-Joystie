...
export function OnboardingAccentFooter({
  ...
  return (
    <>
      ...
      <div
        className={`pointer-events-none absolute left-v03-gutter z-[45] flex w-v03-content flex-col items-center gap-[15px] pt-5 ${className}`}
      ...
      >
        <button
          ...
          onClick={onClick}
          disabled={disabled}
          className={`pointer-events-auto inline-flex h-[55px] w-full items-center justify-center gap-2 overflow-hidden rounded-v03-button bg-v03-accent px-[15px] py-2 text-center font-simpler text-[18px] font-bold leading-normal text-[#031D15] shadow-v03-button transition hover:brightness-105 ${
            disabled ? 'opacity-50' : ''
          }`}
        >
          {children}
