...
export function LoginScreen({
  ...
  return (
    <div
      dir="rtl"
      className="v03-funnel-screen absolute inset-x-0 top-0 z-[10] min-h-full overflow-y-auto v03-scroll-hidden"
    >
      <SignupHeroFrame scrollTop={0} />

      <form
        onSubmit={onSubmit}
        className="relative z-[20] mx-auto flex min-h-full w-v03-content flex-col items-center gap-[15px]"
        style={{
          marginTop: SIGNUP_FORM_CONTENT_MARGIN_TOP_PX,
      ...
      >
        ...
        <div className="mt-auto flex w-full flex-col items-center gap-[15px] pt-5">
          {loginError ? (
            <p className="w-full text-center font-simpler text-sm text-red-300">{loginError}</p>
          ) : null}

          <button
            type="submit"
            disabled={formLocked}
            className={`inline-flex h-[55px] w-full items-center justify-center gap-2 overflow-hidden rounded-[22px] bg-white px-[15px] py-2 text-center font-simpler text-[18px] font-bold leading-[21.6px] text-v03-green-900 shadow-v03-button transition hover:brightness-95 ${
          ...
          <p className="text-right font-simpler text-[16px] font-normal leading-[21.6px] text-white">
